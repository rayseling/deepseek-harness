...
`unprovable` is published rather than computed and dropped: it rides `SettingsDescriptor` and the wire view `SettingsNamespaceView`, omitted when empty. What a client owes it depends on how that client writes. A scope-backed surface refuses it: `SettingsScopeController` publishes such a namespace `unavailable` and read-only and declines writes, because it resolves a whole section and would persist one built on an incomplete read. A path-addressed form — the Models pages — keeps editing it, and is safe without the check: `pathOps` names only the fields the card observed, so a withheld subtree is absent from both sides of its diff and no op sets or unsets it.
...
A recursive `z.lazy` schema is covered in both directions — one carrying a secret is withheld and reported without exhausting the stack, and one that merely recurses returns its value — and through each relation the cycle can close by: an object property, an `array` element (the shape a recursive tree takes), and a `union` branch, since each reaches the lazy node by a different path into the relation set — and a `dict` whose KEY schema declares the secret is withheld with the key name absent from the value and from `secrets`, while an ordinary key schema keeps the dict readable.
