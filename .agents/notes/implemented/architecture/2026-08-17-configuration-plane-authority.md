...
Widening the carrier is only half of it: the browser has to know. `host.describe` reports `remoteConfiguration`, registered by the carrier that owns the policy, and the client's `ConnectionHandle` exposes `canConfigure()` — loopback, or that capability. Settings persistence and the welcome acknowledgement select their mode through that call rather than the page's hostname, re-read on every load so a remote page upgrades from process-local to Host when the handshake lands instead of being pinned at construction. Without it the carrier would answer `settings.describe` while every remote page kept writing to memory, which is a widened plane nobody can use. `settings.openDocument` stays keyed to `isLoopback` on purpose: the native editor opens on the Host's screen, so a remote operator has nothing to look at.
