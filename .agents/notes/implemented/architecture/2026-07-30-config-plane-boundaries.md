...
**Reading configuration is as privileged as writing it.** `settings.describe` and `credentials.describe` join the privileged set, so the whole configuration plane answers only a loopback Host by default — a deployment may widen it to its declared authorities, which is still not authentication ([the configuration plane follows trustedHosts on request](2026-08-17-configuration-plane-authority.md)). The model catalog (`llm.providers`, `llm.models`) deliberately does not: it carries provider ids, display names, and model lists — no endpoints, no key state — and a LAN client's model picker needs it. The boundary is asserted over a real HTTP server rather than a hand-assembled request, because the `Host` header a browser actually sends is what decides it.
...
- **Fixing the redaction gaps here** — five defects in what the plane hands back belong to redaction, not to a boundary change: a secret reachable only through a union, intersection, transform, or tuple; a secret field's `.default(...)` inside `schema.toJSON()`; write-rejection text that may quote the input; the client rehydrating the envelope through schemastery's `new Function`; and pi-ai's plain-string `headers` dict, which can legitimately hold `Authorization`. Taking them on alongside the boundary would have half-fixed each. The value guarantee belongs to [fail-closed secret redaction](2026-08-17-fail-closed-secret-redaction.md): a subtree whose redaction cannot be proven is withheld with its position published in `unprovable`, pi-ai's `headers` element carries `role('secret')` so header values are write-only on the wire, and the redacting describe sanitizes the `schema.toJSON()` envelope. The rejection text and the executable envelope stay in the owning READMEs' Known Limitations.

## Consequences

A LAN client on a `trustedHosts` deployment cannot render the settings page under the default posture; loopback is the configuration surface unless the deployment sets `privilegedAuthority` ([the configuration plane follows trustedHosts on request](2026-08-17-configuration-plane-authority.md)). A plugin that registers a settings namespace is not web-configurable until it also registers a configurable provider — deliberate, and the reason `settings-not-exposed` names the boundary in its message. `SettingsDescriptor` gained a required `revision`, so any programmatic constructor of a descriptor-shaped value must supply it, and `settings/document-updated` is a new event any provider-side listener may now observe. Clients that ignore `expectedRevision` keep last-write-wins semantics unchanged. Deferred: a wire describe that refuses an unprovable namespace outright instead of serving a value with a withheld subtree ([fail-closed secret redaction](2026-08-17-fail-closed-secret-redaction.md) reports the position without refusing), and a non-executable browser schema protocol.
