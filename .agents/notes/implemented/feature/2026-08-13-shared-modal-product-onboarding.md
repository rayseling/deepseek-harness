...
**The welcome notice reuses the existing durable field.** Its exact copy and version live in `onboarding-copy.ts`. Clients that may reach the configuration plane compare and write `ui-onboarding.welcomeNoticeVersion` through the existing settings API, and only Continue acknowledges the current version. That set is loopback under the shipped default and follows the handshake's `remoteConfiguration` capability when a deployment widens it ([the configuration plane follows trustedHosts on request](../architecture/2026-08-17-configuration-plane-authority.md)). Under the shipped default a remote client retains the existing process-local fallback, because the settings namespace is loopback-only there. No Host schema, API-proxy allowlist, or persistence implementation changes.
