...
> The carrier-level decisions below remain current, and so does `--host` itself; the all-interface *value* this note admitted is superseded by [the CLI refuses the wildcard bind host](../simplification/2026-08-13-cli-refuses-wildcard-host.md), which made `--host 0.0.0.0` a usage error and left all-interface binding to composition.
