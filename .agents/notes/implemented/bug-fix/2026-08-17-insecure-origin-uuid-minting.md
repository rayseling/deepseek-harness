...
`dsh web` with an all-interfaces bind is usable from a browser: the readiness handshake, every unary call, the downlink upgrades, and image attachment all mint ids on an insecure origin. Loopback behaviour is unchanged, because `getRandomValues` is available there too and the id format is identical.
...
The existing insecure-origin assertion for the generic channel (`packages/client/connection/tests/client-apply.client.spec.ts`) is unchanged and still passes over the shared helper, which is what shows the deletion of the package-private copy preserved that path. `packages/client/connection` and `packages/client/ui-conversation` suites pass unchanged.

No snapshot changed. The transcripts and web goldens record a booted application over loopback, which is the configuration they already ran in; the behaviour this fix restores appears only under a non-loopback origin, which no keyless fixture serves. Reproducing it end-to-end needs a real browser on a second host, and the assembled-application harness has no lane for that today — a named gap, and the reason the unit assertion pins the environment rather than the deployment.
