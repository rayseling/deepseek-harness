...
这类部署上的浏览器确实能完成启动：id 铸造不依赖安全上下文（[非安全源 UUID 铸造](2026-08-17-insecure-origin-uuid-minting.md)），那是本决策之前就存在的另一个缺陷。特权方法的钉定在那里依然把配置面限制为回环专属，所以远程浏览器可以对话、可以跑工具，但不能重新配置这套部署。
