...
Withholding is not refusal on the Host: the descriptor and its wire view both carry `unprovable` beside the redacted layers, and the Host still answers. The client is where it becomes refusal — `SettingsScopeController` publishes such a namespace as `unavailable` and read-only, and refuses writes on it, because a value with subtrees the redactor could not prove secret-free is not one a form may edit. What remains deferred is a Host-side refusal that never serves the namespace at all.
