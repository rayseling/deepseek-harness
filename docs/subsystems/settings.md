...
Withholding is not refusal on the Host: the descriptor and its wire view both carry `unprovable` beside the redacted layers, and the Host still answers. The client is where it becomes refusal, for the surfaces that resolve a whole section: `SettingsScopeController` publishes such a namespace as `unavailable` and read-only and refuses writes, since it would persist a section built on an incomplete read. A path-addressed form keeps editing it safely, because its ops name only the fields it observed and a withheld subtree is absent from both sides of the diff. What remains deferred is a Host-side refusal that never serves the namespace at all.
