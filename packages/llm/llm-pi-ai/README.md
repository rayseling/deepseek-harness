...
- **`headers` values are write-only on the wire** — nothing in a header value tells a redactor which entry is a credential, so the dict's element carries `role('secret')`: a redacted `describe()` withholds every value and reports the key names in its `secrets` sidecar, and a configuration UI renders write-only inputs rather than the current values. Credentials still belong in `apiKeyEnv` references, which stay readable as reference names. Requests use the real values; only the wire view is stripped.
