...
- **`headers` 的值在协议上是只写的**：header 值本身没有任何信息能告诉脱敏器哪一条是凭据，因此该字典的元素带 `role('secret')`：脱敏后的 `describe()` 会扣下每一个值、只在 `secrets` 附带清单里报出键名，配置界面渲染的是只写输入框而不是当前值。凭据仍应放在 `apiKeyEnv` 引用中，那些引用名依然可读。真实请求使用真实值，只有协议视图被剥离。
