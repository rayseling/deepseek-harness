...
- **A key the schema does not declare rides the wire verbatim** — the object walker preserves keys outside the schema (a field from a newer plugin version, or a typo), so redaction cannot classify them and a credential stored under one is returned as it is. Redaction is schema-declared by construction: model a secret as `role('secret')` on a declared field, or as an `apiKeyEnv`-style reference.
- **Refusal is the client's, not the Host's** — values are fail-closed: the walker follows `object`/`dict`/`array` and enumerates each declared secret, and a `role('secret')` reachable only through a `union`, `intersect`, `transform`, `tuple`, or `lazy` builder, sitting under a malformed container value, or declared on a `dict`'s KEY schema, has its subtree withheld with the position reported in `unprovable`. `sanitizeSchemaEnvelope` also strips `default`/`initial` from every secret ref of the serialized schema, so a secret's declared default no longer rides the envelope. What is deferred is Host-side refusal: `describe` still answers a namespace carrying `unprovable`, and only a scope-backed client turns that into an unavailable, read-only scope (`SettingsScopeController`); a path-addressed form keeps editing it, safely, because its ops name only the fields it observed. A `describeForWire()` that refuses such a namespace outright, and that sanitizes error text, is still the fuller answer.
