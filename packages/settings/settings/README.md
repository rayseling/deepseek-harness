...
- **`schema.toJSON()` still carries a secret field's `.default(...)`** to every client, and neither `describe` nor `register` rejects a schema that does that. Values themselves are now fail-closed: the walker follows `object`/`dict`/`array`, and a `role('secret')` reachable only through a `union`, `intersect`, `transform`, or `tuple` has its subtree withheld with the position reported in `unprovable`, so no caller receives a value whose redaction could not be proven. What remains deferred is the surfacing half — a `describeForWire()` that refuses such a namespace outright instead of serving it with a hole, and that sanitizes the serialized envelope and error text.
