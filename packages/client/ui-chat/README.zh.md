...
该偏好从每个已认证页面经 Host 持久化，仅在设置表单无法持久化写入时才只在当前进程内生效。明确选择会立即更新所有使用方；收到 Host 已接受的设置后会同步当前值。
