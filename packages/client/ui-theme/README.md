...
The service provides itself immediately with the schema defaults, then loads the `ui-theme` namespace and writes each accepted theme or font-size change through the Host settings API. Pushed settings changes and reconnects refetch the namespace. The [Host-backed preferences note](../../../.agents/notes/implemented/bug-fix/2026-08-06-host-backed-web-preferences.md) owns that persistence lifecycle, and the [authenticated-page settings note](../../../.agents/notes/implemented/feature/2026-08-31-host-settings-on-every-authenticated-page.md) owns which pages use it.
