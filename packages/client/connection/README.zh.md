...
一个页面是否可以抵达配置面，由 `connection.canConfigure()` 决定，而不是由它的 hostname 决定：回环始终可以，远程页面则要看 Host 的握手是否报出 `remoteConfiguration`——那是载体依据自己的 `privilegedAuthority` 注册、并经 `host.describe` 发布的 capability。设置的持久化在每次 load 时读取它，因此一个以仅本进程起步的页面会在握手抵达后升级。`isLoopback` 保留给那些确实需要同一台机器的场景，例如在 Host 的原生编辑器中打开文档。
