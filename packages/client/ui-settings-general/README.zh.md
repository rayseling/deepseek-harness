...
在回环浏览器上，只有当宿主确认可准备好一份由提供方持有的本地文档时，外壳才渲染**打开配置文件**。该操作会在原生文本编辑器中打开该文档（macOS 上绕过浏览器文件关联）。远程浏览器从不注册该操作，但 Host 方法不检查来源，因此任何持有浏览器会话的页面仍能直接调用 `settings/openSettingsDocument`。
