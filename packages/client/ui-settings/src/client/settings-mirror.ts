...
export interface SettingsMirrorSnapshot {
  /**
   * `unavailable` is the process-local state, which a later handshake may
   * leave; `ready` persists across later failed refreshes (the held view keeps
   * serving); `idle` means no answer is held and no read is running, so
   * `ensure` will start one.
   */
  status: 'idle' | 'loading' | 'ready' | 'unavailable'
  /**
   * `host` reads and writes the Host document; `memory` keeps the page
   * process-local. Live: a remote page starts `memory` and moves to `host`
   * once the handshake reports the deployment widened its configuration plane.
   */
  mode: 'host' | 'memory'
...
export class SettingsDescribeMirror implements SettingsDescribeFace {
  ...
  /**
   * @param persistence - live mode selector, re-read on every read entry: a
   * remote page is process-local until the handshake reports the deployment
   * answers the configuration plane to it.
  ...
  constructor(
    ...
    this.store = createSnapshotStore<SettingsMirrorSnapshot>({
      status: mode === 'host' ? 'idle' : 'unavailable',
      view: undefined,
      error: null,
      mode,
    })
   ...
   */
  load(): Promise<void> {
    if (this.syncMode() === 'memory') return Promise.resolve()
  ...
  /**
   * Adopt the page's current configuration authority, reading only when it just
   * widened. The Host description republishes on every generation, so an
   * unchanged mode must not cost a wire read.
   */
  adoptMode(): void {
    const before = this.getSnapshot().mode
    if (this.syncMode() === 'host' && before === 'memory') void this.load()
  }

  /**
   * Adopt the live mode, publishing the transition when it moved. A downgrade
   * also bumps the read generation, so a read started under Host authority
   * cannot publish a document into a page that just lost it.
   * @returns the mode now in force.
   */
  private syncMode(): 'host' | 'memory' {
    const mode = this.persistence()
    const before = this.store.getSnapshot()
    if (mode === before.mode) return mode
    this.generation += 1
    this.store.set(mode === 'host'
      ? { ...before, status: 'idle', mode }
      : { status: 'unavailable', view: undefined, error: null, mode })
    return mode
  }
  ...
  private async run(): Promise<void> {
    ...
    try {
      do {
        const before = this.store.getSnapshot()
        // A downgrade landing between reruns retires the loop; `load` already
        // published the process-local snapshot.
        if (before.mode === 'memory') break
        ...
        if ('view' in outcome) {
          this.store.set({ ...held, status: 'ready', view: outcome.view, error: null })
        } else {
