...
export class SettingsScopeController<T> implements SettingsScope<T> {
   ...
   * @param mirror - the shared describe mirror this scope derives from; the
   * mirror's live mode is also this scope's, so a page the deployment does not
   * answer the configuration plane for stays process-local.
  ...
  constructor(
    ...
    // Subscribed in memory mode too: the mirror publishes the mode change when
    // a late handshake widens the deployment, and this scope must leave
    // 'unavailable' with it.
  ...
  private derive(): void {
    ...
    if (mirrored.mode !== this.getSnapshot().mode) {
      // A late handshake moved the mirror between Host and process-local;
      // adopt it, dropping any Host answer a downgraded page may no longer show.
      this.store.update((draft) => {
        draft.mode = mirrored.mode
        draft.status = mirrored.mode === 'host' ? 'loading' : 'unavailable'
        if (mirrored.mode === 'host') return
    ...
    if (mirrored.mode === 'memory' || mirrored.view === undefined) return
