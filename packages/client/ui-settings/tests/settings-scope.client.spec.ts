...
describe('SettingsScopeController', () => {
  ...
  it('refuses a namespace whose Host view reports withheld subtrees', async () => {
    ...
    const describeCall = vi.fn()
      .mockResolvedValue(ok({ writable: true, hasDocument: true, namespaces: [withheld] }))
    const mutate = vi.fn()
  ...
  it('leaves memory mode when a late handshake widens the deployment', async () => {
    ...
    const mirror = new SettingsDescribeMirror(wire, () => authority)
    ...
    await vi.waitFor(() => {
      expect(scope.getSnapshot()).toMatchObject({
        status: 'ready', mode: 'host', value: { preference: 'dark' }, revision: 1,
      })
    })
    await scope.dispose()
  })

  it('drops the Host answer when the page loses configuration authority', async () => {
    const describeCall = vi.fn().mockResolvedValue(described({ preference: 'dark' }, 1))
    const mutate = vi.fn()
    ...
    await mirror.load()
    expect(scope.getSnapshot()).toMatchObject({
      status: 'unavailable', mode: 'memory', value: undefined,
      base: undefined, user: undefined, revision: undefined, writable: false,
    })
    await scope.set('preference', 'light')
    expect(mutate).not.toHaveBeenCalled()
    await scope.dispose()
