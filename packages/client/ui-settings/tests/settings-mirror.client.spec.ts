...
describe('SettingsDescribeMirror', () => {
  ...
  it('memory persistence stays unavailable and never touches the wire', async () => {
    const describeCall = vi.fn()
    const mirror = new SettingsDescribeMirror(
      { settings: { describe: describeCall } } as never, () => 'memory')
    await mirror.ensure()
    await mirror.load()
    expect(mirror.getSnapshot()).toEqual({
      status: 'unavailable', view: undefined, error: null, mode: 'memory',
    })
    expect(describeCall).not.toHaveBeenCalled()
  })

  it('adoptMode reads once the page gains configuration authority, and not before', async () => {
    const describeCall = vi.fn().mockResolvedValue(described([view('theme', 1)]))
    let authority: 'host' | 'memory' = 'memory'
    const mirror = new SettingsDescribeMirror(
      { settings: { describe: describeCall } } as never, () => authority)
    // An unchanged authority must not turn every description generation into a read.
    mirror.adoptMode()
    mirror.adoptMode()
    expect(describeCall).not.toHaveBeenCalled()
    ...
    mirror.adoptMode()
    await vi.waitFor(() => {
      expect(mirror.getSnapshot()).toMatchObject({ status: 'ready', mode: 'host' })
    })
    expect(describeCall).toHaveBeenCalledOnce()
    // Already host: a later description generation costs nothing.
    mirror.adoptMode()
    expect(describeCall).toHaveBeenCalledOnce()
  })

  it('drops the held view when the page loses configuration authority', async () => {
    const describeCall = vi.fn().mockResolvedValue(described([view('theme', 1)]))
    let authority: 'host' | 'memory' = 'host'
    const mirror = new SettingsDescribeMirror(
      { settings: { describe: describeCall } } as never, () => authority)
    await mirror.load()
    expect(mirror.getSnapshot()).toMatchObject({ status: 'ready', mode: 'host' })
  ...
  it('discards a read started before the page lost configuration authority', async () => {
    const gate = deferred<RpcResponse<SettingsDescribeView>>()
    const describeCall = vi.fn().mockReturnValueOnce(gate.promise)
    let authority: 'host' | 'memory' = 'host'
    const mirror = new SettingsDescribeMirror(
      { settings: { describe: describeCall } } as never, () => authority)
    const reading = mirror.load()
  ...
  it('retires a pending rerun when the page lost authority mid-read', async () => {
    const gate = deferred<RpcResponse<SettingsDescribeView>>()
    const describeCall = vi.fn().mockReturnValueOnce(gate.promise)
    let authority: 'host' | 'memory' = 'host'
    const mirror = new SettingsDescribeMirror(
    ...
    const reading = mirror.load()
    await vi.waitFor(() => { expect(describeCall).toHaveBeenCalledOnce() })
    // An invalidation mid-read normally earns one rerun…
    void mirror.load()
    authority = 'memory'
    await mirror.load()
    gate.resolve(described([view('theme', 1)]))
    ...
    // …but the rerun must not become a read the page is no longer allowed.
    expect(describeCall).toHaveBeenCalledOnce()
    expect(mirror.getSnapshot()).toEqual({
      status: 'unavailable', view: undefined, error: null, mode: 'memory',
    })
  ...
  it('acceptView before any answer is a no-op instead of inventing a document', () => {
    ...
    expect(mirror.getSnapshot()).toEqual({ status: 'idle', view: undefined, error: null, mode: 'host' })
  })
